...
class YOLOv8_ObjectCounter(YOLOv8_ObjectDetector):
    def __init__(self, model_file='yolov8l.pt', labels=None, classes=[0, 1, 2, 3, 5, 7], conf=0.4, iou=0.5, track_max_age=45, track_min_hits=15, track_iou_threshold=0.3):
    ...
    def predict_video(self, frame_provider, output_file_path, frame_interval=0.1, save_interval=120, verbose=True):
        """Predict frames and handle saving/upload logic"""
        self.reset_tracker()
        ...
        def process_frame(frame):
            ...
            now = datetime.now()
            current_time_str = now.replace(minute=0, second=0, microsecond=0).strftime("%H:%M")
        ...
        while True:
            try:
                ...
                if current_time - last_save_time >= save_interval:
                    logging.info("Save interval reached. Saving counts to Excel.")
                    self.save_count_to_excel(output_file_path)
                ...
                if time_until_next_hour <= 10:  
                    logging.info(f"Waiting for the next hour to upload data. Time until next hour: {time_until_next_hour} seconds")
                    time.sleep(time_until_next_hour)
                    logging.info(f"{datetime.now().strftime('%H:%M')} - Uploading data to Google Drive for the past hour.")
                    self.upload_to_google_drive(output_file_path)
    ...
    def save_count_to_excel(self, output_file_path):
    ...
    def upload_to_google_drive(self, output_file_path):
        """Upload data to Google Drive at the beginning of each full hour."""
    ...
    def save_aggregated_data_to_excel(self, aggregated_counts, output_file_path):
        ...
        for class_name, count in aggregated_counts.items():
            key = (City, code_panel, class_name, now.date(), hour_str)
